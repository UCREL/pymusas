...
- Added the extra group `neural` to the [pyproject.toml](./pyproject.toml) so that the required `torch` and `transformers` libraries are installed allowing the neural models to run.
