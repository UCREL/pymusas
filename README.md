...
Each lexeme (lemma, POS label) in a lexicon may contain one or more semantic tags. When the lexeme contains more than one semantic tag they are ranked in left to right order, whereby the left most tag is the most probable tag based on frequency.
