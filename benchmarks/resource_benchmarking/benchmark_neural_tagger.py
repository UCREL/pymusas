...
language_code_help = (
    "The language code of the neural tagger to benchmark. "
...
tagger_size_help = (
    "The size of the neural tagger to benchmark within the given `language_code`."
)
output_file_help = (
...
device_help = (
    "The device to use for the neural tagger. This should be a "
...
def main(language_code: benchmarking_utils.LanguageCodes = typer.Argument(help=language_code_help),
    ...
    * Loads the neural tagger for the specified language code and tagger size.
    * Downloads a sufficient number of Wikipedia articles to reach the token limit.
    * Processes the downloaded articles and tags the text using the neural tagger.
    ...
    * Appends the benchmark statistics to the specified output file in JSON format.
    ...
    output_statistics = {
        ...
        tagger_name_key: benchmarking_utils.LANGUAGE_CODE_SIZE_TO_NEURAL_MODEL_NAME[language_code + "_" + tagger_size],
    ...
    with benchmarking_utils.track_memory_usage(load_memory_requirements_key, load_gpu_requirements_key, output_statistics, device):
        benchmarking_utils.load_neural_tagger(language_code, tagger_size, device)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with benchmarking_utils.track_memory_usage(average_memory_required_key, average_gpu_memory_required_key, output_statistics, device):
            total_times = timeit.repeat(stmt='benchmarking_utils.tagger_speed_test(neural_spacy_tagger, Path(temp_dir), temp_file_prefix)', 
                                        setup='neural_spacy_tagger = benchmarking_utils.load_neural_tagger(language_code, tagger_size, device)',
        ...
        with benchmarking_utils.track_memory_usage(large_text_memory_requirements_key, large_gpu_memory_requirements_key, output_statistics, device):
            tagger = benchmarking_utils.load_neural_tagger(language_code, tagger_size, device)
            tagger(large_text)
